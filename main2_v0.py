...
class sevenpixels:
	def __init__(self):
		...
		for i in range(1,2):
			self.grd_dict.append(cv2.imread(f'files/seven_pixels_training/jhn_grd.jpg'))
			
	
	def pixel1(self,pixel1_w,pixel1_b):
		#print(len(self.original_img_dict))
		if str(type(self.pooled_a0)) != "<class 'numpy.ndarray'>":
			for img_num,img in enumerate(self.original_img_dict):
				a0 = img[:,:,:]
				a0 = a0/255
				#pixel1_z1 = layer1_a0 * layer1_w + layer1_b
				self.pooled_a0.append(mypooling(a0,3))
			self.pooled_a0 = np.array(self.pooled_a0).astype(np.float32)
		
		
		a0 = self.pooled_a0[0].astype(np.float32)	
	...
	def layer1(self):
		self.layer1_pixels_output = [] #layer1 7a1's
		self.pooled_a1 = []
		for i in range(7):
			self.layer1_pixels_output.append(self.pixel1(self.pixel1s_w[i],self.pixel1s_b[i]))
		self.layer1_a1 = np.array(self.layer1_pixels_output)
		
		for i in range(7):
			self.pooled_a1.append(mypooling(self.layer1_a1[i],3)) 
		self.pooled_a1 = np.array(self.pooled_a1)
	...
	def pixel2(self,pixel2_w,pixel2_b):
		...
		a1 = self.pooled_a1 #[(height*width*3),.....] #7neurons
		print('its layer2::::')
		...
		pixel2_a2 = 1/(1+np.exp(-pixel2_z2))
		return pixel2_a2 #shape(height-2,width-2,no_channel)
	
	def layer2(self):
		self.layer2_pixels_output = []
		self.pooled_a2 = []
		for i in range(7):
			self.layer2_pixels_output.append(self.pixel2(self.pixel2s_w[i],self.pixel2s_b[i])) #each self.pixel2s_w[i] will have 7 elements or filter of 3*3
		self.layer2_a2 = np.array(self.layer2_pixels_output)
		print(self.layer2_a2.shape)
		for i in range(7):
			self.pooled_a2.append(mypooling(self.layer2_a2[i],3))
		self.pooled_a2 = np.array(self.pooled_a2)
		return self.layer2_a2
	
	def pixel3(self,pixel3_w,pixel3_b):
		a2 = self.pooled_a2 #7neurons 
		...
		pixel3_a3 = 1/(1+np.exp(-pixel3_z3))
		return pixel3_a3 #shape(height-2,width-2,3)
	
	def layer3(self):
		self.layer3_pixels_output = []
		self.pooled_a3 = []
		for i in range(7):
			self.layer3_pixels_output.append(self.pixel3(self.pixel3s_w[i],self.pixel3s_b[i])) #each self.pixel3s_w[i] will have 7 elements or filter of 3*3
		self.layer3_a3 = np.array(self.layer3_pixels_output)
		
		for i in range(7):
			self.pooled_a3.append(mypooling(self.layer3_a3[i],3))
		self.pooled_a3 = np.array(self.pooled_a3)
	...
	def pixel4(self,pixel4_w,pixel4_b):
		...
		pixel4_a4 = 1/(1+np.exp(-pixel4_z4))
		return pixel4_a4 #shape(height-2,width-2,3)
		
	def layer4(self):
		self.layer4_pixels_output = []
		self.pooled_a4 = []
		for i in range(7):
			self.layer4_pixels_output.append(self.pixel4(self.pixel4s_w[i],self.pixel4s_b[i])) #each self.pixel3s_w[i] will have 7 elements or filter of 3*3
		self.layer4_a4 = np.array(self.layer4_pixels_output)
		
		for i in range(7):
			self.pooled_a4.append(mypooling(self.layer4_a4[i],3))
		self.pooled_a4 = np.array(self.pooled_a4)
	...
	def pixel5(self,pixel5_w,pixel5_b):
		...
		pixel5_a5 = 1/(1+np.exp(-pixel5_z5))
		return pixel5_a5 #shape(height-2,width-2,3)
		
	def layer5(self):
		self.layer5_pixels_output = []
		self.pooled_a5 = []
		for i in range(7):
			self.layer5_pixels_output.append(self.pixel5(self.pixel5s_w[i],self.pixel5s_b[i])) #each self.pixel3s_w[i] will have 7 elements or filter of 3*3
		self.layer5_a5 = np.array(self.layer5_pixels_output)
		
		for i in range(7):
			self.pooled_a5.append(mypooling(self.layer5_a5[i],3))
		self.pooled_a5 = np.array(self.pooled_a5)
	...
	def pixel6(self,pixel6_w,pixel6_b):
		...
		pixel6_a6 = 1/(1+np.exp(-pixel6_z6))
		return pixel6_a6 #shape(height-2,width-2,3)
		
	def layer6(self):
		self.layer6_pixels_output = []
		self.pooled_a6 = []
		for i in range(7):
			self.layer6_pixels_output.append(self.pixel6(self.pixel6s_w[i],self.pixel6s_b[i])) #each self.pixel3s_w[i] will have 7 elements or filter of 3*3
		self.layer6_a6 = np.array(self.layer6_pixels_output)
		
		for i in range(7):
			self.pooled_a6.append(mypooling(self.layer6_a6[i],3))
		self.pooled_a6 = np.array(self.pooled_a6)
	...
	def layer7(self):
		self.layer7_pixels_output = []
		self.pooled_a7 = []
		for i in range(1):
			self.layer7_pixels_output.append(self.pixel7(self.pixel7s_w[i],self.pixel7s_b[i])) #each self.pixel3s_w[i] will have 7 elements or filter of 3*3
		self.layer7_a7 = np.array(self.layer7_pixels_output)
		
		for i in range(1):
			self.pooled_a7.append(mypooling(self.layer7_a7[i],3))
		self.pooled_a7 = np.array(self.pooled_a7)
	...
	def backward_update(self):
		...
		self.dz7 = (anti_pooling(self.pooled_a7[0]) - self.grd_dict[0]).astype(np.float32)
		print(self.dz7.shape)
		
		#dw7 = dz7 * a6
		self.dw7 = [] #list of 7 elements where each element is a 3*3 after coorelation #with 3channels
		for i in range(7):
			self.dw7.append(mycoorelation(self.dz7,self.pooled_a6[i]))		
		...
		for i in range(7):
			self.dz6.append(anti_pooling(self.da6[i]) * (anti_pooling(self.pooled_a6[i]) * (1 - anti_pooling(self.pooled_a6[i]))))
		...
		for y in range(7):
			self.dw6.append([])
			for i in range(7):
				self.dw6[y].append(mycoorelation(self.dz6[y],self.pooled_a5[i]))
		...
		for i in range(7):
			self.dz5.append(anti_pooling(self.da5[i]) * (anti_pooling(self.pooled_a5[i]) * (1 - anti_pooling(self.pooled_a5[i]))))
		...
		for y in range(7):
			self.dw5.append([])
			for i in range(7):
				self.dw5[y].append(mycoorelation(self.dz5[y],self.pooled_a4[i]))
		...
		self.dz4 = []
		for i in range(7):
			self.dz4.append(anti_pooling(self.da4[i]) * (anti_pooling(self.pooled_a4[i]) * (1 - anti_pooling(self.pooled_a4[i]))))
		...
		for y in range(7):
			self.dw4.append([])
			for i in range(7):
				self.dw4[y].append(mycoorelation(self.dz4[y],self.pooled_a3[i]))
		...
		self.dz3 = []
		for i in range(7):
			self.dz3.append(anti_pooling(self.da3[i]) * (anti_pooling(self.pooled_a3[i]) * (1 - anti_pooling(self.pooled_a3[i]))))
		...
		for y in range(7):
			self.dw3.append([])
			for i in range(7):
				self.dw3[y].append(mycoorelation(self.dz3[y],self.pooled_a2[i]))
		...
		self.dz2 = []
		for i in range(7):
			self.dz2.append(anti_pooling(self.da2[i]) * (anti_pooling(self.pooled_a2[i]) * (1 - anti_pooling(self.pooled_a2[i]))))
		...
		for y in range(7):
			self.dw2.append([])
			for i in range(7):
				self.dw2[y].append(mycoorelation(self.dz2[y],self.pooled_a1[i]))
		...
		self.dz1 = []
		for i in range(7):
			self.dz1.append(anti_pooling(self.da1[i]) * (anti_pooling(self.pooled_a1[i]) * (1 - anti_pooling(self.pooled_a1[i]))))
		...
		for y in range(7):
			self.dw1.append([])
			for i in range(1):
				self.dw1[y].append(mycoorelation(self.dz1[y],self.pooled_a0[i]))
	...
	def status_update(self,forward_layer,countt_update):
		...
		f = 'files/test_result/7pixels/checkpoint.npz'
		np.savez(f,pixel1s_w = self.pixel1s_w,
				...
				pooled_a0 = self.pooled_a0,
				pooled_a1 = self.pooled_a1,
				pooled_a2 = self.pooled_a2,
				pooled_a3 = self.pooled_a3,
				pooled_a4 = self.pooled_a4,
				pooled_a5 = self.pooled_a5,
				pooled_a6 = self.pooled_a6,
				pooled_a7 = self.pooled_a7,			
			)
	...
	def show(self):
		try:
			for i in range(7):
				plt.subplot(1,7,i+1)
				plt.imshow(self.pooled_a1[i])
				plt.title(f'layer1_a1_{i-1}')
			plt.show()
			
			for i in range(7):
				plt.subplot(1,7,i+1)
				plt.imshow(self.pooled_a2[i])
				plt.title(f'layer2_a2_{i-1}')
			plt.show()
			
			for i in range(7):
				plt.subplot(1,7,i+1)
				plt.imshow(self.pooled_a3[i])
				plt.title(f'layer3_a3_{i-1}')
			plt.show()
			
			for i in range(7):
				plt.subplot(1,7,i+1)
				plt.imshow(self.pooled_a4[i])
				plt.title(f'layer4_a4_{i-1}')
			plt.show()
			
			for i in range(7):
				plt.subplot(1,7,i+1)
				plt.imshow(self.pooled_a5[i])
				plt.title(f'layer5_a5_{i-1}')
			plt.show()
			
			for i in range(7):
				plt.subplot(1,7,i+1)
				plt.imshow(self.pooled_a6[i])
				plt.title(f'layer6_a6_{i-1}')
			plt.show()
				
			plt.subplot(1,1,1)
			plt.imshow(self.pooled_a7[0])
	...
	def initiate(self):
		if os.path.exists('files/test_result/7pixels/checkpoint.npz'):
			checkpoint = np.load('files/test_result/7pixels/checkpoint.npz') 
			...
			self.pooled_a0 = checkpoint['pooled_a0']
			self.pooled_a1 = checkpoint['pooled_a1']
			self.pooled_a2 = checkpoint['pooled_a2']
			self.pooled_a3 = checkpoint['pooled_a3']
			self.pooled_a4 = checkpoint['pooled_a4']
			self.pooled_a5 = checkpoint['pooled_a5']
			self.pooled_a6 = checkpoint['pooled_a6']
			self.pooled_a7 = checkpoint['pooled_a7']
			print(type(self.pooled_a1),len(self.pooled_a1),self.pooled_a1.shape)
			#self.processing_stage = 'FORWARD_C'
			...
			self.pixel1s_w = np.random.random((7,3,3,1)).astype(np.float32)
			"""
			...
			"""
			self.pixel1s_b = np.random.random((7,1)).astype(np.float32)
			
			self.pixel2s_w = np.random.random((7,7,3,3,1)).astype(np.float32)
			self.pixel2s_b = np.random.random((7,7,1)).astype(np.float32)
			
			self.pixel3s_w = np.random.random((7,7,3,3,1)).astype(np.float32)
			self.pixel3s_b = np.random.random((7,7,1)).astype(np.float32)
			
			self.pixel4s_w = np.random.random((7,7,3,3,1)).astype(np.float32)
			self.pixel4s_b = np.random.random((7,7,1)).astype(np.float32)
			
			self.pixel5s_w = np.random.random((7,7,3,3,1)).astype(np.float32)
			self.pixel5s_b = np.random.random((7,7,1)).astype(np.float32)
			
			self.pixel6s_w = np.random.random((7,7,3,3,1)).astype(np.float32)
			self.pixel6s_b = np.random.random((7,7,1)).astype(np.float32)
			
			self.pixel7s_w = np.random.random((1,7,3,3,1)).astype(np.float32)
			self.pixel7s_b = np.random.random((1,7,1)).astype(np.float32)
